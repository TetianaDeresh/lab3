...
const int Final[64] = { 	40 ,8  ,48 ,16 ,56 ,24 ,64 ,32 ,
...
void encrypt(const string& plain_txt, const string& key)
{
    ...
    L_key[0] = shift_bit(key_firstHalf, Shift[0]);
    R_key[0] = shift_bit(key_secondHalf, Shift[0]);

    
    for (int i = 1; i < 16; i++) {
        L_key[i] = shift_bit(L_key[i - 1], Shift[i]);
        R_key[i] = shift_bit(R_key[i - 1], Shift[i]);
    }

    
    string key_48[16];
    for (int i = 0; i < 16; i++) {
        string combined_key = L_key[i] + R_key[i];
        key_48[i] = "";
        for (int j = 0; j < 48; j++)
            key_48[i] += combined_key[pc_2[j] - 1];
    }
    ...
    string IP_permuted = "";
    for (int i = 0; i < 64; i++) 
        IP_permuted += plain_txt_64[IP[i] - 1];

   
    string L = IP_permuted.substr(0, 32);
    string R = IP_permuted.substr(32, 32);
    ...
    string RL = R_32[15] + L_32[15];

    
    string encrypted_bin = "";
    for (int i = 0; i < 64; i++) 
        encrypted_bin += RL[Final[i] - 1];

    
    cout << "Encrypted Text: " << Bin_to_Hex(encrypted_bin) << endl;
}
...
void decrypt(const string& cipher_txt, const string& key)
{
    ...
    string IP_permuted = "";
    for (int i = 0; i < 64; i++) 
        IP_permuted += cipher_txt_64[IP[i] - 1];
    ...
    string decrypted_bin = "";
    for (int i = 0; i < 64; i++) 
        decrypted_bin += RL[Final[i] - 1];

    
    cout << "Decrypted Text: " << Bin_to_Hex(decrypted_bin) << endl;
}
...
int main() {
    ...
    cout << "Enter the 16-character Hexadecimal key: ";
    cin >> key;
    cout << "Enter the text (Hexadecimal) to process: ";
    cin >> txt;
